...
using System;
using System.Collections.Generic;  // <- Diferencia
...
class Ejercicio166Ht
{
    static void Main()
    {
        ...
        Dictionary<string,string> englishSpanish = 
            new Dictionary<string,string>();   // <- Diferencia
    ...
    static void verTodas(Dictionary<string,string> englishSpanish)
    {
        foreach (KeyValuePair<string,string> clave in englishSpanish)  // <- Diferencia
        {
            Console.WriteLine("{0}  -->  {1}",
                clave.Key, clave.Value);  // <- Diferencia
        }
    }
